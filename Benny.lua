...
-- BUTTON 3 AUTO CLICK A
local ClickABtn = Instance.new("TextButton")
ClickABtn.Parent = MainFrame
ClickABtn.Position = UDim2.new(0,15,0,116)
ClickABtn.Size = UDim2.new(1,-30,0,30)
ClickABtn.Text = "AUTO CLICK A : OFF"
ClickABtn.Font = Enum.Font.GothamBold
ClickABtn.TextColor3 = Color3.new(1,1,1)
ClickABtn.BackgroundColor3 = Color3.fromRGB(110,50,180)

Instance.new("UICorner", ClickABtn)

ClickABtn.MouseButton1Click:Connect(function()
	ClickSound:Play()

	AutoClickA = not AutoClickA

	if AutoClickA then
		ClickABtn.Text = "AUTO CLICK A : ON"
		Status.Text = "Status: Auto Click A Active"
		Notify("⌨️ AUTO CLICK A ON")
		...
		task.spawn(function()
			while AutoClickA do
				...
				task.wait(1) -- klik tombol A setiap 1 detik
			end
		end)
	else
		ClickABtn.Text = "AUTO CLICK A : OFF"
		Status.Text = "Status: Auto Click A Disabled"
		Notify("❌ AUTO CLICK A OFF")
	end
end)
